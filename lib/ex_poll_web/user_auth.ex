defmodule ExPollWeb.UserAuth do
  use ExPollWeb, :verified_routes

  import Plug.Conn
  import Phoenix.Controller

  alias ExPoll.Accounts
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      ExPollWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_user:
      ...
      defmodule ExPollWeb.PageLive do
        use ExPollWeb, :live_view

        on_mount {ExPollWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{ExPollWeb.UserAuth, :ensure_authenticated}] do
  ...
  defp authorized?(user, path, method, event \\ nil) when is_binary(path) do
    role = user && user.role
    uri = URI.parse(path)
    method = String.upcase(method || "GET")
    info = ExPollWeb.Router.route_info(method, uri.path, uri.host)
    route = info.route
